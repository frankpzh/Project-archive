...
static inline void
glamo_reg_write(unsigned short reg, unsigned short val)
{
	IO_WRITES(GLAMO_BASE + reg, val);
}

static inline unsigned short
glamo_reg_read(unsigned short reg)
{
	return IO_READS(GLAMO_BASE + reg);
}
 ...
 * framebuffer
 */
static unsigned short u16a_lcd_init[] = {
...
static unsigned short u16a_gen_init_0x0000[] = {
...
static unsigned short u16a_gen_init_0x0200[] = {
...
#define glamofb_cmdq_empty() (glamo_reg_read(GLAMO_REG_LCD_STATUS1) & (1 << 15))
...
void glamofb_cmd_mode(int on)
{
	if (on) {
		...
		glamo_reg_write(GLAMO_REG_LCD_COMMAND1,
			  GLAMO_LCD_CMD_TYPE_DISP |
			  GLAMO_LCD_CMD_DATA_FIRE_VSYNC);

		while (!(glamo_reg_read(GLAMO_REG_LCD_STATUS2) & (1 << 12)));
		DELAY(15000); /* you really need this ;-) */
	} else {
		/* RGB interface needs vsync/hsync */
		if (glamo_reg_read(GLAMO_REG_LCD_MODE3) & GLAMO_LCD_MODE3_RGB)
			glamo_reg_write(GLAMO_REG_LCD_COMMAND1,
				  GLAMO_LCD_CMD_TYPE_DISP |
...
void glamofb_cmd_write(unsigned short val)
{
	while (!glamofb_cmdq_empty());
	glamo_reg_write(GLAMO_REG_LCD_COMMAND1, val);
}
...
void glamo_init()
{
	...
	printf("Glamo core device ID: 0x%x, Revision 0x%x\n",
