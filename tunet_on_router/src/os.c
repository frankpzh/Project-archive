...
BOOL socket_fetch_DWORD(SOCKET sock, BUFFER *buf, BYTE **ptr, DWORD *d)
{
	...
	while (!buffer_has_data(buf, p, sizeof(DWORD))) {
...
BOOL socket_fetch_BYTE(SOCKET sock, BUFFER *buf, BYTE **ptr, BYTE *b)
{
	...
	while (!buffer_has_data(buf, p, sizeof(BYTE))) {
		len = os_socket_tcp_recv_nonblock(sock, tmpbuf, sizeof(tmpbuf));
		buffer_append(buf, tmpbuf, len);
	}
...
BOOL socket_fetch_bytes(SOCKET sock, BUFFER *buf, BYTE **ptr, BYTE *bytes, INT len1)
{
	int len;
	BYTE tmpbuf[1024];
	BYTE *p = *ptr;
